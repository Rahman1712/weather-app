...
function App() {
  ...
  const searchPressed = async () => {
    console.log(`${url}weather?q=${search}&units=metric&appid=${api}`);
    try {
      setLoading(true);
      ...
      toast.error('😶‍🌫️ Please enter a correct place or country name', {
        position: 'top-right',
        autoClose: 5000,
  ...
  return (
    <div className='home-container min-h-screen flex flex-col items-center justify-between'>
        ...
        <div className='card p-6 rounded-md shadow-md max-w-sm mx-auto' style={{ width: '400px', height: '400px', overflow: 'auto' }}>
          ...
          {!weather.name ? (
            <div className='mt-6'>
              ...
              Welcome to Weather Check - your destination for live weather information!
              </p>
            </div>
          ) : (
      ...
      <footer className='my-2 mx-3 text-sm text-gray-500 self-end cursor-pointer d-flex justify-content-end align-items-center' onClick={handleClick}>
        ...
        <span className="d-sm-none">Abdul Rahman</span>
      </footer>
